...
elif touch -am %s %s >/dev/null 2>&1 && test ! -f %s && test ! -f %s && test -f %s; then\n\
  ...
  if (file_size_limit == 0 || part_number == 1)
    {
      ...
      if (vanilla_operation_mode)
	fprintf (output, "\
mkdir _sh%05d || ( echo '%s' '_sh%05d'; exit 1 )\n",
		 sharpid, N_("failed to create locking directory"), sharpid);
      else
	fprintf (output, "\
...
/* Ridiculously enough.  FIXME: No fixed limit in GNU...  */
     ...
     const char *path;
{
  ...
  cursor = mkdir_already[mkdir_already_count++] = xstrdup (path);
...
if test ! -f _sh%05d/new; then\n\
  ...
  $echo '%s' '%s' '%s'\n",
	  ...
	  && md5_stream (fp, md5buffer) == 0)
	{
	  ...
	  fprintf (output, "\
  if ( %s --help </dev/null 2>&1 | grep 'sage: md5sum \\[' ) >/dev/null 2>&1 \\\n\
  && ( %s --version </dev/null 2>&1 | grep -v 'textutils 1.12' ) >/dev/null; then\n\
    %s -c << %s >/dev/null 2>&1 \\\n\
    || $echo '%s:' '%s'\n",
		   MD5SUM_COMMAND, MD5SUM_COMMAND, MD5SUM_COMMAND,
		   DEFAULT_HERE_DELIMITER, restore_name,
		   N_("MD5 check failed"));
	  ...
	  fprintf (output, " %c%s\n\
%s\n",
		   ' ', restore_name, DEFAULT_HERE_DELIMITER);
     ...
     char *const *argv;
{
  ...
  while (optchar = getopt_long (argc, argv,
				"+$BCDFL:MPQSTVXZab:cd:fg:hjl:mn:o:pqs:wxz",
				long_options, NULL),
	 optchar != EOF)
    switch (optchar)
      {
...
exit 0\n",
	   N_("lock directory"), sharpid, sharpid, N_("removed"),
	   N_("failed to remove"));
